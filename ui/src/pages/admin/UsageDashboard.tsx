...
import type { AdminUsageResponse } from '../../types';
 ...
 * Ensures the chart X-axis is complete and evenly spaced even when
...
export default function UsageDashboard() {
  ...
  if (loading) return <LoadingSpinner message="Loading usage analytics..." />;
  if (error) return <ErrorAlert message={error} onRetry={fetchUsage} />;
  if (!usage) return null;

  const bucketLabels = generateBucketLabels(period);

  // Pivot timeline: one row per timestamp, one column per user
  const userNames = [...new Set(usage.timeline.map((p) => p.user_label))];
  const makeUserRow = (label: string) => {
    const row: Record<string, string | number> = { label };
    for (const u of userNames) row[u] = 0;
    return row;
  };
  const byTimestamp = new Map<string, Record<string, string | number>>(
    bucketLabels.map((label) => [label, makeUserRow(label)]),
  );
  for (const point of usage.timeline) {
    const label = formatBucketLabel(parseUTC(point.timestamp), period);
    if (!byTimestamp.has(label)) {
      byTimestamp.set(label, makeUserRow(label));
    }
    byTimestamp.get(label)![point.user_label] = point.requests;
  }
  const timelineData = [...byTimestamp.values()];

  // Pie: requests by user
  const pieData = usage.by_user.map((u) => ({
    name: u.user_label,
    value: u.requests,
  }));

  // Token timeline: aggregate input/output across all users per bucket
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
        <h1 style={{ margin: 0 }}>Usage Analytics</h1>
      ...
      {/* Charts — Requests by user */}
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '2rem', marginBottom: '2rem' }}>
        <div style={{ background: colors.cardBg, border: `1px solid ${colors.cardBorder}`, borderRadius: 8, padding: '1rem' }}>
          <UsageTimelineChart data={timelineData} models={userNames} title="Requests by User Over Time" />
        </div>
        <div style={{ background: colors.cardBg, border: `1px solid ${colors.cardBorder}`, borderRadius: 8, padding: '1rem' }}>
          <UsagePieChart data={pieData} title="Requests by User" />
        </div>
      </div>
      ...
      {usage.by_user.length > 0 && (
        <div style={{ background: colors.cardBg, border: `1px solid ${colors.cardBorder}`, borderRadius: 8, padding: '1rem' }}>
          <h3 style={{ margin: '0 0 0.75rem', fontSize: '1rem' }}>Per-User Breakdown</h3>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {usage.by_user.map((u) => (
                <tr key={u.user_label}>
                  <td style={tdStyle}>{u.user_label}</td>
                  <td style={{ ...tdStyle, textAlign: 'right' }}>{formatNumber(u.requests)}</td>
                  <td style={{ ...tdStyle, textAlign: 'right' }}>{formatNumber(u.input_tokens)}</td>
                  <td style={{ ...tdStyle, textAlign: 'right' }}>{formatNumber(u.output_tokens)}</td>
                  <td style={{ ...tdStyle, textAlign: 'right' }}>{formatNumber(u.input_tokens + u.output_tokens)}</td>
                </tr>
              ))}
            </tbody>
          </table>
